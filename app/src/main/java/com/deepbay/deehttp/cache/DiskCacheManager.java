package com.deepbay.deehttp.cache;

import android.content.Context;
import android.os.Environment;
import com.deepbay.deehttp.DayuApp;
...
/**
 * 参考:https://github.com/JakeWharton/DiskLruCache
 * 磁盘缓存工具类
 */
...
public class DiskCacheManager {
    ...
    public DiskCacheManager(Context context, String uniqueName) {
        try {
            ...
            mDiskLruCache = DiskLruCache.open(cacheFile, DayuApp.getAppVersionCode(), 1, CacheConstants.CACHE_MAXSIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * @return 加密结果
     */
    private static String getMD5Result(String value) {
        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
            md.update(value.getBytes("UTF-8"));
            byte[] result = md.digest();
            return getDeeString(result);
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
            return CacheConstants.NOSUCHALGORITHM;
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            return CacheConstants.UNSUPPENCODING;
        }
    }

    private static String getDeeString(byte[] result) {
